...
class Compose(object):
    """
    Composes several imgseg_transforms together.

    Args:
        imgseg_transforms (List[Transform]): list of imgseg_transforms to compose.

    Example:
        imgseg_transforms.Compose([
            imgseg_transforms.CenterCrop(10),
            imgseg_transforms.ToTensor(),
        ])
    """

    def __init__(self, imgseg_transforms):
        self.imgseg_transforms = imgseg_transforms

    def __call__(self, img, seg):
        for t in self.imgseg_transforms:
            img, seg = t(img, seg)
        return img, seg
...
class ToTensor(object):
    ...
    def __call__(self, pic, label):
        if not isinstance(pic, np.ndarray) or not isinstance(label, np.ndarray):
            raise (RuntimeError("imgseg_transforms.ToTensor() only handle np.ndarray"
                                "[eg: data readed by cv2.imread()].\n"))
        if len(pic.shape) > 3 or len(pic.shape) < 2:
            raise (RuntimeError("imgseg_transforms.ToTensor() only handle np.ndarray with 3 dims or 2 dims.\n"))
        if len(pic.shape) == 2:
            pic = np.expand_dims(pic, axis=2)
        if not len(label.shape) == 2:
            raise (RuntimeError("imgseg_transforms.ToTensor() only handle np.ndarray seglabel with 2 dims.\n"))

        img = torch.from_numpy(pic.transpose((2, 0, 1)))
        if not isinstance(img, torch.FloatTensor):
            img = img.float()
        seg = torch.from_numpy(label)
        if not isinstance(seg, torch.LongTensor):
            seg = seg.long()
        return img, seg
...
class Crop(object):
    ...
    def __call__(self, img, seg):
        ...
        if pad_height > 0 or pad_width > 0:
            ...
            seg = cv2.copyMakeBorder(seg, pad_h_half, pad_height - pad_h_half, pad_w_half, pad_width - pad_w_half,
        ...
        seg = seg[h_off:h_off + self.crop_H, w_off:w_off + self.crop_W]
        return img, seg
...
class RandScale(object):
    ...
    def __call__(self, img, seg):
        ...
        seg = cv2.resize(seg, None, fx=scale_factor_x, fy=scale_factor_y, interpolation=cv2.INTER_NEAREST)
        return img, seg
